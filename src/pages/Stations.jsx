...
export default function Stations() {
  ...
  if (!MapsRef.current && Platform.OS !== 'web') {
    try { MapsRef.current = require('react-native-maps'); } catch {}
  }
  const MapView = MapsRef.current?.default;
  const Marker = MapsRef.current?.Marker;
  const Polyline = MapsRef.current?.Polyline;
  ...
  const getToken = async () => Platform.OS === 'web' ? window.localStorage.getItem('token') : await AsyncStorage.getItem('token');
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      try {
        mapRef.current.fitToCoordinates(routeCoords, { edgePadding: { top: 40, right: 40, bottom: 40, left: 40 }, animated: true });
      } catch {}
    }, 50);
  }, [routeCoords]);
        ...
        <View style={styles.mapWrap}>
          {MapView ? (
            <MapView
              ref={mapRef}
              style={styles.map}
              initialRegion={{ latitude: -23.55, longitude: -46.63, latitudeDelta: 0.1, longitudeDelta: 0.1 }}
            >
              {endpoints.start && Marker && <Marker coordinate={endpoints.start} title="Início" />}
              {endpoints.end && Marker && <Marker coordinate={endpoints.end} title="Fim" />}
              {routeCoords.length > 0 && Polyline && (
                <Polyline coordinates={routeCoords} strokeColor="#FFA652" strokeWidth={4} />
              )}
            </MapView>
          ) : (
            <View style={[styles.map, { alignItems: 'center', justifyContent: 'center' }]}>
              <Text style={styles.mute}>Mapa não disponível no web sem configuração adicional. Rode no Android/iOS ou peça integração com Google Maps web.</Text>
            </View>
          )}
        </View>
      </ScrollView>
